...
import { timeToColspan, getStartSlotIndex, computeEnd, DAYS, TIME_SLOTS, HOURS_FOR_HEADER } from './gen-schedule-helper';
...
export function gen_cell(cls: ClassEntry): string {
  const span = timeToColspan(cls.start, cls.duration);
...
export function gen_roll(day: string, classes: ClassEntry[], isLastRow: boolean): string {
  ...
  let slotPtr = 0;
  const sortedClasses = classes.filter(cls => cls.day === day).sort((a, b) => a.start.localeCompare(b.start));
  for (const cls of sortedClasses) {
    const clsStartSlotIndex = getStartSlotIndex(cls.start);
    const colspan = timeToColspan(cls.start, cls.duration);
